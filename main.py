from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 
def reset_timer():
    global reps
    window.after_cancel(timer)
    label_timmer.config(text='Timer', fg=GREEN)
    canvas.itemconfig(timer_text, text='00:00')
    label_check.config(text='')
    reps = 0

# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_countdown():
    global reps
    reps += 1

    work_secs = WORK_MIN * 60
    short_secs = SHORT_BREAK_MIN * 60
    long_secs = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        count_down(long_secs)
        label_timmer.config(text='Break', fg=RED)
    elif reps % 2 == 0:
        count_down(short_secs)
        label_timmer.config(text='Break', fg=PINK)
    else:
        count_down(work_secs)
        label_timmer.config(text='Work', fg=GREEN)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    mins = math.floor(count / 60)
    segs = count % 60
    if segs < 10:
        segs = f'0{segs}'

    canvas.itemconfig(timer_text, text=f'{mins}:{segs}')
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count -1)
    else:
        start_countdown()
        mark = ''
        for i in range(math.floor(reps/2)):
            mark += '✔'
            label_check.config(text=mark)
            if mark == '✔✔✔✔':
                mark = ''


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title('Pomodoro')
window.config(padx=100, pady=50, bg=YELLOW)

#label timmer
label_timmer = Label(text='Timer', bg=YELLOW, fg=GREEN, font=(FONT_NAME, 40, 'bold'))
label_timmer.grid(column=1,row=0)

#label check
label_check = Label(bg=YELLOW, fg=GREEN, font=(FONT_NAME, 10, 'bold'))
label_check.grid(column=1,row=4)

#button start
button_start = Button(text='Start', command=start_countdown)
button_start.grid(column=0,row=3)

#button reset
button_reset = Button(text='Reset', command=reset_timer)
button_reset.grid(column=2,row=3)

#imagen
canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
imagen = PhotoImage(file='tomato.png')
canvas.create_image(100, 112, image=imagen)
timer_text = canvas.create_text(100, 130, text='00:00', fill='white', font=(FONT_NAME, 28, 'bold'))
canvas.grid(column=1,row=1)




window.mainloop()